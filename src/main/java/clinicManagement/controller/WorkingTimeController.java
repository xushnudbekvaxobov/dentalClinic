...
public class WorkingTimeController {
    ...
    public ResponseEntity<ApiResponse<?>> getLastWeekWorkingTime(@PathVariable UUID id){
        return workingTimeService.getLastWeekWorkingTimeByDoctorId(id);
    }

    @GetMapping("/present-week-working-time/{id}")
    public ResponseEntity<ApiResponse<?>> getPresentWeekWorkingTime(@PathVariable UUID id){
        return workingTimeService.getPresentWeekWorkingTimeByDoctorId(id);
    }

    @GetMapping("/next-week-working-time/{id}")
    public ResponseEntity<ApiResponse<?>> getNextWeekWorkingTime(@PathVariable UUID id){
        return workingTimeService.getNextWeekWorkingTimeByDoctorId(id);
    }

    @GetMapping("/present-month-working-time/{id}")
    public ResponseEntity<ApiResponse<?>> getPresentMonthWorkingTime(@PathVariable UUID id){
        return workingTimeService.getPresentMonthWorkingTimeByDoctorId(id);
    }

    @GetMapping("/last-month-working-time/{id}")
    public ResponseEntity<ApiResponse<?>> getLastMonthWorkingTime(@PathVariable UUID id){
        return workingTimeService.getLastMonthWorkingTimeByDoctorId(id);
    }


    @GetMapping("/one-day-working-time/{id}/{date}")
    public ResponseEntity<ApiResponse<?>> getDayWorkingTime(@PathVariable UUID id,@PathVariable LocalDate workingDate){
        return workingTimeService.oneDayWorkingTimeByDoctorId(id,workingDate);
    }
    
    @PostMapping("/add-working-time/{id}")
    public ResponseEntity<ApiResponse<?>> addWorkingTime(@RequestBody WorkingTimeDto workingTimeDto,@PathVariable UUID id){
        return workingTimeService.addWorkingTime(workingTimeDto,id);
    }

    @PutMapping("/update/{id}")
    public ResponseEntity<ApiResponse<?>> update(@RequestBody WorkingTimeDto workingTimeDto,@PathVariable UUID id){
